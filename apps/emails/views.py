...
class EmailTemplateListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['templates'] = EmailTemplate.objects.filter(is_active=True).order_by('category', 'name')
        return context
...
class EmailTemplatePreviewView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        sample_context = {
            ...
            'reset_link': self.request.build_absolute_uri('/auth/password-reset/confirm/'),
        }
...
def email_preview_html(request, template_key):
    ...
    sample_context = {
        ...
        'user_name': request.user.get_full_name() if request.user.is_authenticated else 'John Doe',
        'site_name': 'Django SaaS Boilerplate',
...
class EmailLogListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['email_logs'] = EmailMessageLog.objects.select_related('template').order_by('-created_at')[:100]
        return context
