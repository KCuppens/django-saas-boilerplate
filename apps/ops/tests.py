...
from .tasks import (
    backup_database, cleanup_old_backups, system_maintenance, 
    health_check_task
)
...
class OpsTasksTestCase(TestCase):
    ...
    def test_backup_database_postgresql(self, mock_makedirs, mock_subprocess, mock_settings):
    ...
    def test_backup_database_sqlite(self, mock_file, mock_call_command, mock_settings):
        ...
        self.assertTrue(result['success'])
        mock_call_command.assert_called_once_with('dumpdata', stdout=mock_file.return_value, indent=2)
    ...
    def test_cleanup_old_backups(self, mock_exists, mock_remove, mock_stat, mock_glob, mock_settings):
    ...
    @patch('apps.ops.tasks.settings')
    def test_system_maintenance_success(self, mock_settings, mock_cache, mock_call_command):
    ...
    @patch('apps.ops.tasks.settings')
    def test_health_check_task_all_healthy(self, mock_settings, mock_connection, mock_cache, mock_disk):
