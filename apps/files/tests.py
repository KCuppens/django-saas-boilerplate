"""Test cases for file upload and management functionality."""
...
class FileServiceTestCase(TestCase):
    ...
    def create_test_file(
        self, filename="test.txt", content=b"test content", mime_type="text/plain"
    ):
    ...
    def test_get_download_url_public_file(self, mock_storage):
        ...
        file_upload = FileUpload(storage_path="uploads/test.jpg", is_public=True)
    ...
    def test_get_download_url_presigned(self, mock_storage):
        ...
        mock_storage.generate_presigned_url.return_value = (
            "https://example.com/presigned/test.jpg"
        )

        file_upload = FileUpload(
            id=uuid.uuid4(), storage_path="uploads/test.jpg", is_public=False
        )
        ...
        with patch("builtins.hasattr", return_value=True):
            ...
            mock_storage.generate_presigned_url.assert_called_once_with(
                "uploads/test.jpg", expires_in=1800, method="GET"
            )
    ...
    def test_get_download_url_fallback(self, mock_reverse, mock_storage):
        ...
        file_upload = FileUpload(
            id="123", storage_path="uploads/test.jpg", is_public=False
        )
    ...
    def test_validate_file_invalid_extension(self):
        ...
        test_file = self.create_test_file(
            "test.xyz", b"test content", "application/octet-stream"
        )
    ...
    def test_validate_file_dangerous_extension(self):
        ...
        test_file = self.create_test_file(
            "malware.exe", b"test content", "application/octet-stream"
        )
    ...
    def test_validate_file_unsupported_mime_type(self):
        ...
        test_file = self.create_test_file(
            "test.txt", b"test content", "application/unknown"
        )
